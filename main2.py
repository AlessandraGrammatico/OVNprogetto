...
#lab 4.6 Create a main that constructs the network defined by ’nodes.json’
# and runs its method stream over 100 connections with signal power equal to 1 mW
# and the input and output nodes randomly chosen. This
# run has to be performed in turn for latency and snr path choice. Accordingly,
...
def main():
    ...
    plt.xlabel('dB')
    plt.savefig('SNRDistributionFullfixed_rate.png')
    plt.show()
    ...
    plt.xlabel('Gbps')
    plt.savefig('BitRateFullfixed_rate.png')
    plt.show()
    ...
    plt.xlabel('dB')
    plt.savefig('SNRDistributionFullflex_rate.png')
    plt.show()
    ...
    plt.savefig('BitRateFullFlex_Rate.png')
    plt.show()
    ...
    plt.xlabel('dB')
    plt.savefig('SNRDistributionFullshannon.png')
    plt.show()
    ...
    plt.savefig('BitRateFullShannon.png')
    plt.show()
    ...
    plt.savefig('LatencyDistribution.png')
    plt.show()
    snrs=[connection.snr for connection in streamed_connections_shannon]
    plt.hist(np.ma.masked_equal(snrs, 0), bins=20)
    plt.title('SNR Distribution')
    plt.savefig('SNRDistribution.png')
    plt.show()
